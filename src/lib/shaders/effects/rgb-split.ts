...
export const rgbSplit = createShaderRecord({
  ...
  variables: [
    ...
    createShaderVariable('splitOffset').asRange('Split Amount', 0.005, 0.0, 0.05, 0.001),
  ...
  body: `
    void main() {
      ...
      float r = texture2D(imageTexture, uv + direction * splitOffset).r;
      float g = texture2D(imageTexture, uv).g;
      float b = texture2D(imageTexture, uv - direction * splitOffset).b;
